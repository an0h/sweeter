defmodule SweeterWeb.CoreComponents do
  @moduledoc """
        ...
        <div class="flex min-h-full items-center justify-center">
          <div class="w-full max-w-3xl p-4 sm:p-6 lg:py-8">
            <.focus_wrap
              ...
              class="shadow-gray-slate-500/10 ring-slate-500/10 relative hidden rounded-2xl bg-gray-800 p-14 shadow-lg ring-1 transition"
            >
  ...
  def button(assigns) do
    ...
    <button
      type={@type}
      class={[
        "phx-submit-loading:opacity-75 rounded-lg bg-slate-900 hover:bg-slate-500 py-2 px-3",
  ...
  def input(%{type: "checkbox", value: value} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      <label class="flex items-center gap-4 text-sm leading-6 text-slate-200">
  ...
  def label(assigns) do
    ~H"""
    <label for={@for} class="block text-sm font-semibold leading-6 text-slate-400">
      <%= render_slot(@inner_block) %>
    </label>
    """
  end
  ...
  def header(assigns) do
    ...
    <header class={[@actions != [] && "flex items-center justify-between gap-6", @class]}>
      <div>
        <h1 class="text-lg font-semibold leading-8 text-slate-400">
          <%= render_slot(@inner_block) %>
        </h1>
        <p :if={@subtitle != []} class="mt-2 text-sm leading-6 text-slate-200">
    ...
    <div class="overflow-y-auto px-4 sm:overflow-visible sm:px-0">
      <table class="w-[40rem] mt-11 sm:w-full">
        ...
        <tbody
          ...
          class="relative divide-y divide-slate-100 border-t border-slate-200 text-sm leading-6 text-slate-500"
        >
          <tr :for={row <- @rows} id={@row_id && @row_id.(row)} class="group hover:bg-slate-50">
            ...
            <td :if={@action != []} class="relative w-14 p-0">
              <div class="relative whitespace-nowrap py-4 text-right text-sm font-medium">
                ...
                <span
                  :for={action <- @action}
                  class="relative ml-4 font-semibold leading-6 text-slate-900 hover:text-slate-500"
                >
    ...
    <div class="mt-14">
      <dl class="-my-4 divide-y divide-slate-100">
        <div :for={item <- @item} class="flex gap-4 py-4 text-sm leading-6 sm:gap-8">
          ...
          <dd class="text-slate-500"><%= render_slot(item) %></dd>
        </div>
      </dl>
    </div>
    """
  ...
  def back(assigns) do
    ...
    <div class="mt-16">
      <.link
        navigate={@navigate}
        class="text-sm font-semibold leading-6 text-slate-900 hover:text-slate-500"
      >
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(SweeterWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(SweeterWeb.Gettext, "errors", msg, opts)
    end
  end
