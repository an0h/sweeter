defmodule SweeterWeb.CityLive.FormComponent do
  ...
  def update(%{city: city} = assigns, socket) do
    changeset = Content.change_city(city)
  ...
  def handle_event("validate", %{"city" => city_params}, socket) do
    changeset =
      socket.assigns.city
      |> Content.change_city(city_params)
  ...
  defp save_city(socket, :edit, city_params) do
    case Content.update_city(socket.assigns.city, city_params) do
      {:ok, city} ->
  ...
  defp save_city(socket, :new, city_params) do
    case Content.create_city(city_params) do
      {:ok, city} ->
        notify_parent({:saved, city})
